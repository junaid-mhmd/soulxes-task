import { Building2, Luggage, ThumbsUp, Ticket, UserRound, Zap } from "lucide-react";
import type {
  FilterGroup,
  FlightCardData,
  PromoCardData,
...
export const flights: FlightCardData[] = [
  {
    ...
    rows: [
      {
        ...
        fromTime: "14:50",
        fromLocation: "Chhatrapati Shivaji Maharaj International (BOM), Mumbai, India",
        duration: "3hr 10min",
        toTime: "18:00",
    ...
    airlineValue: "indigo",
    infoBarItems: [
      { icon: Ticket, label: "Separate tickets booked together for cheaper price" },
    ...
    rows: [
      {
        ...
        hasBaggage: true,
        hasInfo: true,
        layoverAfter: "Change of Terminal • Change of planes • 2h 15m Layover in Abu Dhabi",
      },
      {
        date: "Mon, 24 Feb 2026",
        fromTime: "14:20",
    ...
    rows: [
      {
        ...
        duration: "3hr 15min",
        toTime: "11:15",
        toLocation: "Chhatrapati Shivaji Maharaj International (BOM), Mumbai, India",
        hasEntertainment: true,
        hasWifi: true,
        hasBaggage: true,
        hasInfo: true,
...
export function deriveFilterGroupsFromFlights(
  ...
  const airlineOptions = Array.from(airlineCounts.entries())
    .map(([value, { label, count, minPrice }]) => ({
      label: `${label} (${count})`,
      price: formatPrice(minPrice),
      value,
    }));
